      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-6 sm:mb-8 border-b border-slate-200 pb-6 sm:pb-8">
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
            ...
            <div className="flex items-center gap-3">
              <Link
                to="/platforms"
                className="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-slate-100 border border-slate-200 text-slate-700 text-xs sm:text-sm font-medium hover:border-brand-red/40 hover:text-brand-red transition-all"
              >
        ...
        <div className="bg-slate-50 border border-slate-200 rounded-2xl p-3.5 sm:p-5 mb-8 shadow-sm">
          <div className="grid grid-cols-1 md:grid-cols-12 gap-3 sm:gap-4">
            ...
            <div className="md:col-span-3">
              ...
              >
                <option value="All">All Targeted Exams</option>
            ...
            <div className="md:col-span-3">
              ...
              >
                {materialCategories.map((cat: string) => (
                  <option key={cat} value={cat}>{cat === 'All' ? 'All Resource Categories' : cat}</option>
                ))}
              </select>
            </div>
        ...
        {filteredMaterials.length === 0 ? (
          <div className="bg-slate-50 border border-slate-200 rounded-2xl p-12 text-center">
            ...
            <h3 className="text-lg font-semibold text-slate-800">No materials match your query</h3>
            ...
            <button
              ...
              className="mt-4 px-4 py-2 bg-slate-900 hover:bg-brand-red text-white rounded-lg text-sm font-medium transition-all"
            >
              Clear Filters
            </button>
          </div>
            ...
            {filteredMaterials.map((item: StudyMaterialItem, idx: number) => (
              ...
              >
                ...
                <div>
                  ...
                  {/* Title */}
                  <h3 className="text-sm font-bold text-slate-900 group-hover:text-brand-red transition-colors line-clamp-2 leading-snug mb-2">
                    {item.title}
                  </h3>
                ...
                <div>
                  {/* Metadata */}
                  <div className="pt-3 border-t border-slate-100 mb-3 flex items-center justify-between text-xs text-slate-400">
                    <span className="flex items-center gap-1">
                      <ShieldCheck className="w-3.5 h-3.5 text-emerald-500" />
                      <span className="text-slate-600 text-[11px]">{item.licenseType}</span>
                    </span>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    <button
                      ...
                      className="inline-flex items-center justify-center gap-1.5 px-3 py-2 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-medium border border-slate-200 transition-all"
                    >
        ...
        {selectedItem && (
          <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
            <div className="bg-white border border-slate-200 rounded-2xl max-w-2xl w-full p-6 max-h-[90vh] overflow-y-auto shadow-2xl">

              <div className="flex items-start justify-between gap-4 mb-4">
                <div>
                  <div className="flex items-center gap-2 mb-2">
                    <span className="px-2.5 py-0.5 rounded text-xs font-semibold bg-red-50 border border-red-100 text-brand-red">
              ...
              {/* Detail list */}
              <div className="space-y-4 bg-slate-50 border border-slate-200 rounded-xl p-4 mb-6 text-xs">
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <span className="text-slate-400 block">Applicable Examination</span>
                    <span className="text-slate-800 font-medium">{selectedItem.examName}</span>
                  </div>
                  <div>
                    <span className="text-slate-400 block">Open License</span>
                    <span className="text-emerald-600 font-medium">{selectedItem.licenseType}</span>
                  </div>
                  <div>
                    <span className="text-slate-400 block">Document Volume</span>
                    <span className="text-slate-800">{selectedItem.pageCount} Pages ({selectedItem.fileSize})</span>
                  </div>
                  <div>
                    <span className="text-slate-400 block">Access Model</span>
                    <span className="text-emerald-600">100% Free Open Educational Resource</span>
                  </div>
                </div>

                <div>
                  <span className="text-slate-400 block mb-1.5">Covered Topics</span>
                  <div className="flex flex-wrap gap-1.5">
                    {selectedItem.keyTopics.map((t: string, idx: number) => (
                      <span key={idx} className="px-2 py-1 rounded bg-white border border-slate-200 text-slate-700">
                        {t}
                      </span>
                    ))}
                  </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 max-h-60 overflow-y-auto">
                    {selectedItem.chapterPdfUrls.map((ch, idx) => (
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="inline-flex items-center justify-between px-3 py-2 rounded-lg bg-white border border-slate-200 hover:border-emerald-300 hover:bg-emerald-50 transition-all text-xs"
                        title="Open chapter PDF directly"
                      >
                        <span className="text-slate-700 truncate pr-2">{ch.chapter}</span>
              ...
              {selectedItem.directPdfUrl && !selectedItem.chapterPdfUrls && (
                <div className="bg-emerald-50 border border-emerald-100 rounded-xl p-4 mb-6 text-center">
                  ...
                  <p className="text-xs text-slate-600 mb-2">Full book direct PDF available</p>
                  <a
                  ...
                  <p className="mt-2 text-[10px] text-slate-500">Opens in new tab. Use browser "Save as..." to save locally.</p>
                </div>
              )}
              ...
              <div className="flex flex-col sm:flex-row items-center justify-between gap-3 pt-4 border-t border-slate-200">
                <button
                  ...
                  className="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-4 py-2 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-medium border border-slate-200 transition-all"
                >
                ...
                <div className="flex items-center gap-3 w-full sm:w-auto">
                  <button
                    onClick={() => setSelectedItem(null)}
                    className="flex-1 sm:flex-none px-4 py-2 rounded-lg bg-slate-100 text-slate-700 hover:bg-slate-200 text-xs font-medium"
                  >
                    Close
                  </button>
                  <a
