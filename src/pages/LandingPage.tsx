      ...
      <section className="py-10 sm:py-16 bg-slate-50/70 border-t border-slate-200">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
            {spotlightMaterials.map((mat) => (
              <div
                key={mat.id}
                className="bg-white rounded-2xl p-4 sm:p-5 border border-slate-200 shadow-xs hover:shadow-md hover:border-brand-red/40 transition-all flex flex-col justify-between space-y-3.5"
              >
                <div className="space-y-2.5">
                  ...
                  <div className="flex flex-wrap gap-1 pt-1">
                    {mat.keyTopics.slice(0, 2).map((t, idx) => (
                      <span key={idx} className="text-[10px] px-2 py-0.5 bg-slate-50 border border-slate-200 rounded text-slate-600">
                        {t}
                      </span>
                    ))}
                  </div>
