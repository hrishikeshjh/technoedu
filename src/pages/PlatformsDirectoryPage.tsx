      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="md:col-span-5">
              ...
              >
                {categories.map((c: string) => (
                  <option key={c} value={c} className="dark:bg-[#111318] dark:text-[#F8FAFC]">{c === 'All' ? 'All Platform Categories' : c}</option>
                ))}
              </select>
            </div>
        ...
        <div className="mb-6">
          <p className="text-sm text-slate-500 dark:text-[#A7AFBD]">
            Showing <span className="text-slate-900 dark:text-[#F8FAFC] font-semibold">{filteredPlatforms.length}</span> platforms
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {filteredPlatforms.map((platform: LearningPlatformInfo) => (
            <div
              key={platform.id}
              className="bg-white dark:bg-[#111318] border border-slate-200 dark:border-[#252932] rounded-2xl p-5 hover:border-brand-red/40 dark:hover:border-brand-red/60 hover:shadow-md transition-all flex flex-col justify-between group"
            >
              <div>
                {/* Header */}
                <div className="flex items-start gap-3 mb-3">
                  <div className="w-10 h-10 rounded-xl bg-red-50 dark:bg-red-950/40 border border-red-100 dark:border-red-900/30 flex items-center justify-center flex-shrink-0">
                ...
                {/* Org */}
                <p className="text-xs text-slate-400 dark:text-[#7F8795] mb-3">
                  by <strong className="text-slate-600 dark:text-[#A7AFBD] font-medium">{platform.organization}</strong>
                </p>
              ...
              {/* Bottom */}
              <div className="pt-3 border-t border-slate-100 dark:border-[#252932]">
                ...
                <a
                  ...
                  className="w-full inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg bg-slate-900 hover:bg-brand-red dark:bg-[#15171C] dark:hover:bg-brand-red dark:border dark:border-[#252932] text-white text-xs font-semibold transition-all group/btn"
                >
